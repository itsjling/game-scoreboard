...
export function Switch(props: SwitchToggleProps) {
  ...
  return (
    <Toggle accessibilityRole="switch" {...rest} ToggleInput={switchInput} />
  );
}
...
function SwitchInput(props: SwitchInputProps) {
  ...
  if (Platform.OS === "web") {
    if (isRTL) {
      outputRange = [+(knobWidth || 0) + offsetRight, offsetLeft];
    } else {
      outputRange = [offsetLeft, +(knobWidth || 0) + offsetRight];
    }
  } else {
    outputRange = [
