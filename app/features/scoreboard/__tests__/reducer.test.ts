...
describe("scoreboardReducer", () => {
  it("adds and removes players", () => {
    ...
    state = scoreboardReducer(state, scoreboardActions.addPlayer("Alex", "#FF3B30", null))
    state = scoreboardReducer(state, scoreboardActions.addPlayer("Bea", "#0A84FF", null))

    expect(state.activeGame.players).toHaveLength(2)
    expect(state.activeGame.players.map((player) => player.name)).toEqual(["Alex", "Bea"])
  ...
  it("handles round transitions and score mutations", () => {
    ...
    state = scoreboardReducer(state, scoreboardActions.incrementPlayerScore(playerId, 3))
    ...
    state = scoreboardReducer(state, scoreboardActions.setPlayerExactScore(playerId, 7))
  ...
  it("creates history and can load from history", () => {
    ...
    state = scoreboardReducer(state, scoreboardActions.loadHistoryGame(startingGameId))
  ...
  it("supports team lifecycle", () => {
    ...
    state = scoreboardReducer(state, scoreboardActions.addTeam("A-Team", "#0A84FF"))

    expect(state.activeGame.teams).toHaveLength(1)

    const teamId = state.activeGame.teams[0].id
    state = scoreboardReducer(state, scoreboardActions.addPlayer("Alex", "#FF3B30", teamId))
