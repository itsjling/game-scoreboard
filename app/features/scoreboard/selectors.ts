import type { AppSettings, GameSnapshot, Player, ScoreboardState, Team } from "./types"

export function getPlayerDisplayScore(player: Player, settings: AppSettings): number {
  if (settings.showPerRoundScores) {
    return player.scores[settings.currentRound - 1] ?? 0
  }
  return player.scores.slice(0, settings.currentRound).reduce((total, score) => total + score, 0)
}
...
export function getSortedPlayers(players: Player[], settings: AppSettings): Player[] {
...
export function getTeamScores(playersByTeam: Record<string, Player[]>, settings: AppSettings) {
...
export function getHistoryGameById(state: ScoreboardState, gameId: string): GameSnapshot | null {
